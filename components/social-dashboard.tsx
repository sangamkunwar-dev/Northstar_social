...
export function SocialDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (postResult.data) setPosts(postResult.data.map((p: any) => ({ id: p.id, title: p.title, description: p.description ?? '', caption: p.caption, hashtags: p.hashtags ?? '', cta: p.call_to_action ?? '', platforms: p.platforms ?? [], status: p.status, date: formatNepalDate(p.scheduled_for || p.created_at) })))
...
const payload = { user_id: user?.id, title: topic.trim(), description: topic.trim(), caption: caption.trim(), hashtags, call_to_action: cta, platforms: channels, status, scheduled_for: scheduledFor, published_at: status === 'published' ? new Date().toISOString() : null }
  ...
  return <>
  {showWelcome && <div className="fixed inset-0 z-[60] grid place-items-center bg-foreground/40 p-6"><div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center shadow-2xl"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-primary text-primary-foreground"><Sparkles size={24}/></div><p className="mt-6 text-sm font-bold uppercase tracking-[0.18em] text-primary">स्वागत छ</p><h1 className="mt-3 text-3xl font-semibold">नर्थस्टार सामाजिक मा स्वागत छ</h1><p className="mt-4 text-sm leading-6 text-muted-foreground">Sangam Kunwar द्वारा निर्मित — तपाईंका विचारलाई प्रकाशित सामग्रीमा बदल्ने शान्त workspace.</p><button onClick={() => setShowWelcome(false)} className="mt-7 w-full rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground">सुरु गर्नुहोस्</button></div></div>}
