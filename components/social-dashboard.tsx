...
const payload = { user_id: user?.id, title: topic.trim(), description: topic.trim(), caption: caption.trim(), hashtags, call_to_action: cta, platforms: channels, status, scheduled_for: scheduledFor, published_at: status === 'published' ? new Date().toISOString() : null }
  ...
  if (!user) return <><WelcomeScreen visible={showWelcome} onContinue={() => setShowWelcome(false)} /><Landing onSignIn={signIn} notice={notice} /></>
  return <>
  <WelcomeScreen visible={showWelcome} onContinue={() => setShowWelcome(false)} />
...
function WelcomeScreen({ visible, onContinue }: { visible: boolean; onContinue: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-foreground/40 p-6">
    <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center shadow-2xl">
      ...
      <h1 className="mt-3 text-3xl font-semibold">नर्थस्टार सामाजिकमा स्वागत छ</h1>
