...
function SupportTickets({ supabase, user, onNotice }: { supabase: ReturnType<typeof createClient> | null; user: User | null; onNotice: (message: string) => void }) {
  ...
  useEffect(() => { if (!supabase || !user) return; supabase.from('support_tickets').select('id,subject,description,status,priority,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).then(({ data }) => data && setItems(data)) }, [supabase, user])
