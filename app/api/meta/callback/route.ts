...
export async function GET(request: Request) {
  ...
  if (!failure) {
    try {
      ...
      if (!tokenResponse.ok || !token.access_token) throw new Error(token.error?.code ? `meta_${token.error.code}` : 'token_exchange_failed')
      ...
      if (!pagesResponse.ok) throw new Error(pages.error?.code ? `meta_${pages.error.code}` : 'pages_request_failed')
